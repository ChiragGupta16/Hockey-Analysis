...
import pickle
import cv2
from utilities import get_center_of_bbox,get_bbox_width,measure_distance,get_foot_position
import numpy as np
import pandas as pd
...
def detect_frames(frames):
    ...
    for i in range (0, len(frames), batch_size):
        batch_detection = model.predict(frames[i:i+batch_size], conf=0.1)
        detections += batch_detection
    return detections

def get_tracks(frames,read_from_stub=False, stub_path=None):
...
def draw_annotations(frames, tracks,team_ball_control):
    ouput_frames = []
    for frame_num, frame in enumerate(frames):
        ...
        for track_id, ball in ball_dict.items():
            frame = draw_traingle(frame, ball["bbox"],(0,255,0))
...
def draw_team_ball_control(frame,frame_num,team_ball_control):
    overlay = frame.copy()
    cv2.rectangle(overlay, (378,56), (560,190), (255,255,255), -1 )
    alpha = 0.8
    ...
    cv2.putText(frame, f" {team_2*100:.2f}%",(410,88), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3)
    cv2.putText(frame, f" {team_1*100:.2f}%",(410,134), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3)
    cv2.putText(frame, f"Possession",(387,182), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3)
...
def transform_point(point,persepctive_trasnformer,pixel_vertices):
...
def add_transformed_position_to_tracks(tracks):
        ...
        pixel_vertices = np.array([[100, 350], 
                               [1410, 350], 
                               [1400, 815], 
                               [470, 755]])
        ...
        for object, object_tracks in tracks.items():
            for frame_num, track in enumerate(object_tracks):
                for track_id, track_info in track.items():
                    ...
                    position_trasnformed = transform_point(position,persepctive_trasnformer,pixel_vertices)
